...
int main(void)
{
	cout << "ServerL is up and running using UDP on port " << SERVERL << ".\n";
	map<string, string> bookList = readData("literature.txt");

	string message = "serverL:";
	...
	{
		string info = UDP_REC(SERVERL);
		//cout << "The info: " << info << ".\n";
		string bookCode = info.substr(0, info.find(','));
		cout << "ServerL received " << bookCode << " code from the Main Server.\n";
		...
		cout << "ServerL finished sending the availability status of code " << bookCode << " to the Main Server using UDP on port " << SERVERMUDP << ".\n";
	}
