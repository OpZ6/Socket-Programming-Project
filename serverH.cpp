...
int main(void)
{
	cout << "ServerH is up and running using UDP on port " << SERVERH << ".\n";
	map<string, string> bookList = readData("history.txt");
	
	string message = "serverH:";
	...
	{
		string info = UDP_REC(SERVERH);
		//cout << "The info: " << info << ".\n";
		string bookCode = info.substr(0, info.find(','));
		cout << "ServerH received " << bookCode << " code from the Main Server.\n";
		...
		cout << "ServerH finished sending the availability status of code " << bookCode << " to the Main Server using UDP on port " << SERVERMUDP << ".\n";
	}
